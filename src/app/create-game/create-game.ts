...
@Component({
  ...
  standalone: true,
  imports: [SelectField, SeasonSelector, ReactiveFormsModule, RouterLink],
...
export class CreateGame {
  ...
  constructor(
    ...
    const league = toSignal(this.form.get('league')!.valueChanges, {
      initialValue: 'competitie',
    });
    this.players = computed(() =>
      (allPlayers() ?? []).filter((p) =>
        p.competitionIds?.includes(competitionId(this.selectedSeason(), league())),
      ),
    );
    effect(() => {
      this.selectedSeason();
  ...
  async submit() {
    ...
    if (
      payload.players?.some((id) => !allowed.has(id)) ||
      payload.events.some((e) => !allowed.has(e.playerId) || !payload.players?.includes(e.playerId))
    ) {
