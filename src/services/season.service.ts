...
import { collection, collectionData, doc, Firestore, writeBatch } from './firebase';
...
export class SeasonService {
  ...
  getSeasons(): Observable<Season[]> {
    return (
      collectionData(collection(this.db, 'seasons'), {
        idField: 'id',
  ...
  async addSeason(name: string) {
    ...
    batch.set(ref, { name });
    batch.set(doc(this.db, 'seasons', LEGACY_SEASON), {
      name: 'Vorig seizoen',
    });
    for (const seasonId of [LEGACY_SEASON, ref.id]) {
      for (const type of ['competitie', 'beker']) {
        batch.set(doc(this.db, 'competitions', competitionId(seasonId, type)), {
          seasonId,
          type,
        });
      }
